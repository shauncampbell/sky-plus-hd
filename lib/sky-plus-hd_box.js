...
var SkyPlusHDBox = function(params) {
	...
	var details = {};
	var services = {};
	var channelList = this.channelList = new SkyPlusHDChannelList(params.region);
		...
		listener.on('notification',function(ev) {
			...
			if (dirty) {
				if (currentState.source === 'broadcast') {
					...
					watching.name = undefined;
					watching.nameLong = undefined;
					watching.show = undefined;
					watching.showDescription = undefined;
				}
	...
	function updateBroadcastInfo() {
		...
		if (channel) {
			...
			watching.name = channel.name;
			watching.nameLong = channel.nameLong;
			watching.number = channel.number;
			
			channel.getSchedule().then(function(schedule) {
				//	Get the current thing thats on.
				for (var i = 0; i < schedule.length; i++)
				{
					var scheduleItem = schedule[i];
					var start = moment(scheduleItem.start);
					var end = moment(scheduleItem.end);
					
					var m = moment().isBetween(start, end);
					
					if (m)
					{
						watching.show = schedule[i].title;
						watching.showDescription = schedule[i].description;
					}
				}
			});
		}	
	...
	function updatePvrInfo() {
		return self.readPlanner(0).then(function(planner) {
			var item;
			for (var i = 0; i < planner.length; i++)
			{
				var recording = planner[i];
				
				if (recording.resource === currentState.uri)
				{
					watching.name = recording.channel.name;
					watching.nameLong = recording.channel.name;
					watching.number = recording.channel.num;
					watching.show = recording.title;
					watching.showDescription = recording.description;
				}
			}
		});
	}
	
	this.checkPowerState = function() {
		return !currentState.standbyState;
	};
